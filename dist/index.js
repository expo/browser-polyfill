  ...
  _load() {
    if (this.src) {
      if (this.src.startsWith && this.src.startsWith('data:')) {
        ...
        reactNative.ImageStore.addImageFromBase64(
          this.src,
          uri => {
            this.src = uri;
            if (this.src.startsWith && !this.src.startsWith('data:')) {
...
global.CanvasRenderingContext2D = global.CanvasRenderingContext2D || Expo2DContext;
...
function checkEmitter() {
  if (
    ...
    window.emitter = new fbemitter.EventEmitter();
  }
}
...
global.TextDecoder = global.TextDecoder || textEncoding.TextDecoder;
global.TextEncoder = global.TextEncoder || textEncoding.TextEncoder;
...
/*
 Window Resize Stub
*/
window.devicePixelRatio = reactNative.PixelRatio.get();
...
if (!global.__EXPO_BROWSER_POLYFILL_RESIZE) {
  ...
  reactNative.Dimensions.addEventListener('change', ({ screen: { width, height, scale } }) => {
